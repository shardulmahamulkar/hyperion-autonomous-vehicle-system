# Experiment: dynamic PWM + servo with forward-bias path planning
...
def turn_smoothly(direction, steps=5, delay=0.1):
    """ Turns the steering gradually using multiple L or R commands """
    if arduino:
        for _ in range(steps):
            if direction == "R":
                ...
                arduino.write(b'L')
            time.sleep(delay)  # Small delay between steps for smooth movement
    debug_log(f"TURNING {direction} ({steps} steps)")
...
while cap.isOpened():
    ...
    if red_detected:
        debug_log("🚨 ALERT: RED CONE DETECTED! STOPPING VEHICLE 🚨")
        if arduino:
            arduino.write(b'S')
    else:
    ...
    if centroids:
        ...
        if dot_x < closest_point[0]:
            ...
            turn_smoothly("R", steps=5, delay=0.1)  # Smooth Right
        elif dot_x > closest_point[0]:
            debug_log("TURNING LEFT")
            turn_smoothly("L", steps=5, delay=0.1)  # Smooth Left
    ...
    if yellow_cones or blue_cones:
        ...
        if pwm_value > STALL_PWM_THRESHOLD:
            if time.time() - last_move_time > STALL_TIME_THRESHOLD:
                stuck = True
                debug_log("🚨 VEHICLE IS STUCK! ATTEMPTING RECOVERY...")
                if arduino:
                    arduino.write(b'REV')
            else:
                stuck = False
